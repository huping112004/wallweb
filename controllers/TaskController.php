    ...
    public function actionSubmit($projectId = null)
    {
        ...
        if (!$projectId) {
            ...
            $projects = Project::find()
                               ...
                               ->asArray()
                               ->all();
            $taskProjects = Project::find()
                ->leftJoin(Task::tableName(), "`$taskTable`.`project_id` = `$projectTable`.`id`")
                ->where([
                    "`$projectTable`.status" => Project::STATUS_VALID,
                    "`$taskTable`.`user_id`" => $this->uid
                ])
                ->groupBy(`$taskTable`.`project_id`)
                ->orderBy("`$taskTable`.`id` desc ")
                ->limit(10)
                ->asArray()
                ->all();
